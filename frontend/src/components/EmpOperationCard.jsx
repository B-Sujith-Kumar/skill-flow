const EmpOperationCard = ({ text, operation }) => {
  const obj = {
    Add: {
  ...
  return (
    <div
      className={`${obj[operation].bgColor} max-w-xs flex items-center justify-center gap-6 py-6 cursor-pointer shadow-lg rounded-lg  transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl min-w-full overflow-hidden sm:px-4 md:px-0 max-sm:px-2`}
    >
      <div className="bg-white px-6 py-4 rounded-full ">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox={`${operation === "Update" ? "0 0 512 512" : "0 0 448 512"}`}
          width={30}
          className=""
          fill={obj[operation].color}
          stroke="#000000"
          strokeWidth={2}
        >
          <path d={obj[operation].path} />
        </svg>
      </div>
...
export default EmpOperationCard;
