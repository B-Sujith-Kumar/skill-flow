...
const JobDetails = ({ jobDetails, isIndividual }) => {
    ...
    const fetchApplicants = async () => {
      try {
        const applicants = await fetch(
          "http://localhost:3000/api/admin/applicants/" + jobDetails.jobid,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="flex flex-row gap-x-12 max-lg:gap-x-8 max-[910px]:flex-col">
      <div className="mb-8 w-2/3 max-[910px]:w-full">
        <div
      ...
      {localStorage.getItem("isAdmin") !== "true" && (
        <div className="w-1/3 max-[910px]:min-w-full">
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-3xl border shadow-lg bg-white px-6 pt-4 pb-3 mt-9">
              ...
              {shuffleArray(job.filter((job) => job.jobid !== currentJob.jobid))
                ...
                .map((jobItem, index) => (
                  ...
                  >
                    <div
                      key={index}
                      className="max-w-md bg-white pt-4 px-3 border-[1.5px] rounded-xl pb-4 hover:shadow-xl cursor-pointer transition duration-300 ease-in-out h-full my-4"
                    >
      ...
      {localStorage.getItem("isAdmin") === "true" && (
        <div className="w-1/3 max-[910px]:min-w-full">
          <div className="rounded-3xl border shadow-lg bg-white px-6 py-4">
            <h2 className="text-xl font-medium text-slate-800">Applicants</h2>
            <div className="flex flex-col gap-4 mt-4">
              {applicantDetails &&
                applicantDetails.slice(0, 2).map((applicant, index) => (
                  <div
                    key={index}
                    className="flex items-center border-b-[1.5px] gap-x-4 pb-3"
                  >
                    <div>
                      <img
